...
<li>根据前端内容改进了一些新接口，添加了用户权限的开启与锁定状态，同时修改删除操作只删除未锁定状态的数据。</li>
<li>修改课程中教师编号为字符串形式，对应教师用户的字符串用户名。</li>
<li>修复了 update 操作中的一个小 bug。</li>

### 2020.7.20 - Demo 3.3.2
<li>修复了小 bug，插入操作现在会判断是否与禁用的数据ID重复。</li>
<li>已知问题: 学习了如何使用 @Param 注解，等接口差不多写完之后重构一下 dao 层的代码。</li>
<li>为 Score 添加了一些新功能，满足前端调用的需求</li>

### 2020.7.25 - Demo 3.3.3
<li>由于学分存在小数，将 Subject 表中的学分改为了 Double</li>
<li>吐血更新数据库，手动加入了 400 多条真实有效的学生、课程和成绩数据</li>

### 2020.7.27 - Demo 3.3.4
<li>尝试部署到服务器失败，明显还是跨域问题，然后还发现另一个问题就是数据库地址只能填 localhost ，不能用 127.0.0.1 代替。</li>
<li>测试了一天半终于找到了原因，原来是因为不仅阿里云需要开放端口，宝塔面板也需要为后端应用放行端口。</li>
<li>修复了成绩数据库的一个小 bug。</li>

### 2020.8.1 - Demo 3.3.5
<li>开始整合 Shiro，数据库表可能会大改。</li>
<li>用 lombok 优化了一些代码，删除了测试阶段残留的代码。</li>
<li>更新了 javadoc</li>

### 2020.8.2 - Demo 3.3.6
<li>因为涉及到权限和角色这两个新增的表以及角色与权限、用户与角色这两个关联的表，数据库做的很大的修改，顺便加入了更多的参数。</li>
<li>已经重构完用户部分的代码，接下来先创建权限和角色相关的代码，将其使用 Shiro 关联起来并测试用户的登录和权限的设置是否可行，最后再修改其余几张表。先 commit 一下，希望最后不要 rollback</li>

### 2020.8.3 - Demo 3.3.7
<li>写了两天终于出稳定版了，现在 Shiro 基本的授权认证已经完成，只需要稍微优化一下即可。</li>

### 2020.8.5 - Demo 3.4.0
<li>遇到了很严重的跨域问题，最主要的是前端的 ajax 请求默认是不能携带 Cookie 信息的，导致 Postman 上测试接口都是正常的但是放到前后端分离的环境下就不行了。Cookie 的问题在前端做了解决，另外的跨域问题通过后端设置了一个自定义的 ShiroFilter 来添加请求头以及处理未授权时的异常报错。一言难尽。
<li>我是真的肝，几天之内就整合了 Shiro 和 Vue 和 SSM，期间遇到了很多关于跨域的问题，好几天晚上都没怎么睡，不过好在问题都解决了。</li>
<li>重构了整个项目的代码，重构了数据库表，加入了比较正统的权限五张表，之前的 User 也算在其中，对于所有 POJO 的抽象基类都加上了新的字段，将 isReal 改为 valid。</li>
<li>根据同学测试的反馈将学号改成了字符串类型，学生信息不再存储年龄，而是前端接收到出生日期之后及时计算出年龄然后显示。</li>
<li>重写了 DefaultWebSessionManage，现在会发送 Cookie 和 Session 双重认证。</li>
<li>完善了 Shiro 的配置，在 securityManager 中加入了自定义的安全认证，在 shiroFilter 中加入了自定义的过滤器。</li>
<li>添加了 GlobalExceptionHandle 全局控制 Shiro 的异常，返回给前端对应信息。</li>
<li>AuthorizingRealm 中将用户对象替换为用户名解决类型转换问题</li>
<li>已知问题: 其实也不算问题，而是过几天需要做的事。首先将权限和用户匹配起来，比如前端添加了一个管理员用户，立刻为其分配管理员权限。后期再考虑同级别的用户不同的自定义权限。还有一个就是由于重构过了，所有的 Controller 都只包含了最基本的增删改查功能，将原先写的一些特殊功能全部删除了，接下来添加自定义功能。</li>

### 2020.8.6 - Demo 3.4.2
<li>更新了挺多东西的，主要把管理员、学生、教师的权限都解决了，同时在创建用户时也调用了角色的 Mapper 将用户与角色绑定。我之前是把角色放在了用户的属性中，这样做确实省了不少事，现在分开导致前端获取数据需要发送不同的请求，网络开销是成倍增长的。</li>
<li>另外，希望什么时候有空能把项目从头到尾再重构一遍，主要是解决 mapper.xml 的报错（其实没有错，是IDEA nt 了，但是看着不舒服）。其次就是使用注解优雅的操作数据库以及接口。等全部基本完工之后再把之前删了又加，加了又删的 javadoc 给加上。</li>

### 2020.8.7 - Demo 3.4.4
<li>调整了 Shiro 配置中登出操作的顺序，排在匿名访问之后，这样不会导致退出的时候清空 session 导致后端给前端发送的数据有误。</li>
<li>为配合前端获取综合数据和用户权限、角色等信息而添加了新的接口。</li>

### 2020.8.8 - Demo 3.4.6
<li>角色和权限方面又根据前端需求加了很多新接口。</li>
<li>项目第二阶段基本完成，过几天开始重构后端代码。</li>
...
<li>最新版本被 rollback 了。导致错误的原因可能是删除了拦截器的关系，现在还是采用自定义拦截器，但是改用继承 user 类的默认拦截器。</li>

### 2020.8.16 - Demo 3.5.0
<li>发现一个 bug 就是 Mybatis 中 set 标签内的 if 在判断条件时会出错，加入参数本身为布尔值的 false 或者数字 0 ，就会直接判定为错误。以后尽量还是不要用 0 这个数字吧。</li>

### 2020.8.19 - Demo 3.5.2
<li>今天读阿里巴巴 Java 开发手册时发现里面讲到 Service 层暴露在外面的只能是接口并且不能有任何修饰符，我突然想起来学长的项目也是这么做的，而我没有用到接口，一开始也想不明白为什么 Controller 层注入接口却能使用实现类中代码，后来知道了这个 Spring 管理的，于是将 Service 层全部重构了顺便加上了 Javadoc。</li>
<li>专门写了个跨域处理工具类，现在不需要在测试和部署的时候频繁修改配置了。</li>

### 2020.8.20 - Demo 3.5.4
<li>使用 Swagger 和 Swagger Bootstrap UI 快速生成了 API 文档，接下来要使用注解优化 API。</li>
<li>重构了一下 POJO 对象</li>
