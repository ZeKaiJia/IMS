...
## 第二版特性
这是学生管理系统第二版(存档)，该版本使用mybatis管理MySQL，实现多种数据库存储功能以及完整的用户操作界面，与第一版相比集成度更高，内容更加丰富，所有数据仅供单个功能的使用而不会一直在程序运行时存在。<br>

## 操作步骤
修改根目录下jdbc.properties中的数据库用户名、密码、地址等信息为您本地的信息，该文件中还附带有三个表格对应的建表MySQL语句等，使用管理软件或命令行创建后即可运行程序，或者使用test类中提供的基本功能测试。
